        ...
        <div class="card-body" style="padding:0px;margin:0px;">
            <div class="table-responsive">
                <table class="table" style="margin-bottom:0px;">
                    ...
                    <tbody><?php
                        $permission = $this->get_data("permission");
                        if($permission->get_controller_views() != array())
                        {
                            foreach($permission->get_controller_views() as $permission_controller_view)
                            {
                                ...
                                {
                                    echo('<td><i class="fa fa-2x fa-toggle-on" name="permission_controller_views" status="granted" id="'.$permission_controller_view->get_id().'"></i></td>');
                                } else {
                                    echo('<td><i class="fa fa-2x fa-toggle-off" name="permission_controller_views" status="denied" id="'.$permission_controller_view->get_id().'"></i></td>');
                                }
                                echo('</tr>');
                            }
                        } else {
